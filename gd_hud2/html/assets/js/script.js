...
window.addEventListener('message', (event) => { const { status, data } = event.data; if (status == "info") { if (typeof data.health === "number") health(parseInt(data.health.toFixed(0))); if (typeof data.armour === "number") armour(parseInt(data.armour.toFixed(0))); if (typeof data.food === "number") food(parseInt(data.food.toFixed(0))); if (typeof data.water === "number") water(parseInt(data.water.toFixed(0))); } if (status == "visible") data ? show_hud() : hide_hud(); if (status == "speedometer") { if (!data.visible) return show_speedometer(false); show_speedometer(true); const speed_num = typeof data.speed === "number" ? parseInt(data.speed.toFixed(0)) : 0, engine_num = typeof data.engine === "number" ? parseInt(data.engine.toFixed(0)) : 0, fuel_num = typeof data.fuel === "number" ? parseInt(data.fuel.toFixed(0)) : 0; if (data.mph) $(".speed span").text("mph"); speed(speed_num); engine(engine_num); fuel(fuel_num); speed_num <= 1 ? ($(".speedometer, .engine, .fuel").fadeOut(300)) : ($(".speedometer, .engine, .fuel").fadeIn(300)); } });
...
window.addEventListener('message', function(event) {
    if (event.data.action === "updateMoney") {
        ...
        document.getElementById('dirty').innerText = '$' + event.data.dirty.toLocaleString();
